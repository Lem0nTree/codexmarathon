...
impl AccountRequestProcessor {
    ...
    /// Shared native authorities used by the embedded CodexMarathon bridge.
    /// These accessors intentionally return the existing Arcs/lock; they do
    /// not construct a second AuthManager or transition guard.
    pub(crate) fn native_auth_manager(&self) -> Arc<AuthManager> {
        Arc::clone(&self.auth_manager)
    }

    pub(crate) fn native_thread_manager(&self) -> Arc<ThreadManager> {
        Arc::clone(&self.thread_manager)
    }

    pub(crate) fn native_auth_transition_lock(&self) -> Arc<Mutex<()>> {
        Arc::clone(&self.auth_transition_lock)
    }
