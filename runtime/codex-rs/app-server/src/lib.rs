...
mod codexmarathon_bridge;
mod codexmarathon_server;
mod command_exec;
mod config_layer;
mod config_manager;
...
pub async fn run_main_with_transport_options(
    ...
    // The Marathon listener is a second local control boundary owned by this
    ...
    let processor_handle = tokio::spawn({
        ...
        let processor_config = Arc::new(config);
        let processor = Arc::new(MessageProcessor::new(MessageProcessorArgs {
            outgoing: outgoing_message_sender,
            analytics_events_client,
            arg0_paths,
            config: Arc::clone(&processor_config),
            config_manager,
            environment_manager,
        ...
        let marathon_server = codexmarathon_server::spawn(&processor, &processor_config);
        ...
        async move {
            ...
            if let Some(marathon_server) = marathon_server {
                marathon_server.shutdown().await;
            }
            info!(
                exit_reason,
