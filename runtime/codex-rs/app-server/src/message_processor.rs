...
pub(crate) struct MessageProcessor {
    ...
    codexmarathon_recovery_bridge: Arc<CodexMarathonRecoveryBridge>,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let marathon_service_slot = Arc::new(std::sync::OnceLock::new());
        let thread_processor = ThreadRequestProcessor::new(
            ...
            config_warnings,
            Arc::clone(&marathon_service_slot),
        );
        ...
        let _ = marathon_service_slot.set(Arc::clone(&marathon_service));
        ...
        let codexmarathon_recovery_bridge =
            Arc::new(CodexMarathonRecoveryBridge::new(Arc::clone(&outgoing)));

        Self {
            outgoing,
            codexmarathon_recovery_bridge,
    ...
    pub(crate) async fn process_client_notification(&self, notification: ClientNotification) {
        match notification {
            ClientNotification::CodexMarathonRecoveryLifecycle(notification) => {
                self.codexmarathon_recovery_bridge
                    .observe_lifecycle_event(notification);
            }
            ClientNotification::CodexMarathonRecoveryReleaseResult(notification) => {
                self.codexmarathon_recovery_bridge
                    .resolve_release(notification)
                    .await;
            }
            notification => {
    ...
    /// Build a Marathon native bridge from this app-server's existing Codex
    /// authorities. The returned value shares the AuthManager, turn watch,
    /// transition lock, and ThreadManager transport cache; it does not create
    /// a parallel runtime authority.
    pub(crate) fn codexmarathon_native_runtime(
        &self,
        auth_config: NativeAuthConfig,
    ) -> Result<CodexNativeRuntime, String> {
        CodexNativeRuntime::from_current_runtime_with_recovery_bridge(
            self.account_processor.native_auth_manager(),
            self.account_processor.native_thread_manager(),
            self.subscribe_running_assistant_turn_count(),
            self.account_processor.native_auth_transition_lock(),
            auth_config,
            Some(self.codexmarathon_recovery_bridge.clone()),
        )
