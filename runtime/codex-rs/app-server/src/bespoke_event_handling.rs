...
pub(crate) async fn apply_bespoke_event_handling(
    ...
    marathon_service: Option<Arc<crate::marathon_service::MarathonService>>,
) {
    let Event {
        id: event_turn_id,
        msg,
    ...
    match msg {
        ...
        EventMsg::TokenCount(token_count_event) => {
            if let Some(service) = marathon_service
                && let Some(rate_limits) = token_count_event.rate_limits.as_ref()
            {
                service.observe_rate_limits(rate_limits);
            }
