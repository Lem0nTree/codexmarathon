...
pub(crate) struct ChatWidget {
    ...
    // Stable correlation for the native Marathon recovery command. It is
    // derived from the failed turn, so duplicate error notifications cannot
    // create a second recovery identity.
    pending_usage_limit_resume_recovery_id: Option<String>,
    // Stable native context captured when the recovery was parked. These
    // values remain unchanged while auth is switched and the controller
    // reconciles the same recovery ID.
    pending_usage_limit_resume_thread_id: Option<String>,
    pending_usage_limit_resume_turn_id: Option<String>,
    // Context for the one synthetic turn after the controller authorizes it.
    active_usage_limit_resume_recovery_id: Option<String>,
    active_usage_limit_resume_thread_id: Option<String>,
    active_usage_limit_resume_turn_id: Option<String>,
    active_usage_limit_resume_dispatch_turn_id: Option<String>,
    ...
    // Set only after Codex reports that the auth reload changed identity. A
    // Marathon release command must not submit before this gate is true.
    usage_limit_resume_auth_reloaded: bool,
