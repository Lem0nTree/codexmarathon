...
impl App {
    ...
    pub(super) async fn handle_app_server_event(
        ...
        // A reconnect or any subsequent app-server event gives queued native
        // recovery observations another chance to cross the typed boundary.
        self.flush_codexmarathon_recovery_events(app_server_client)
            .await;
    }
    ...
    async fn handle_server_notification_event(
        ...
        // The embedded Marathon runtime sends only an authorization command;
        // the ChatWidget remains the sole owner of the pending prompt and
        // normal input queue. A typed acknowledgement lets the native bridge
        // report rejection/uncertainty without inventing another queue.
        if let ServerNotification::CodexMarathonRecoveryRelease(command) = &notification {
            let released = self.chat_widget.release_usage_limit_resume_turn(
                command.recovery_id.as_str(),
                command.thread_id.as_deref(),
            );
            let (outcome, error_code, error_message) = if released {
                ("released".to_string(), None, None)
            } else {
                (
                    "rejected".to_string(),
                    Some("tui_recovery_gate_rejected".to_string()),
                    Some(
                        "the active Codex TUI has no matching identity-gated pending recovery"
                            .to_string(),
                    ),
                )
            };
            if let Err(error) = app_server_client
                .notify(ClientNotification::CodexMarathonRecoveryReleaseResult(
                    CodexMarathonRecoveryReleaseResultNotification {
                ...
                .await
            {
                tracing::warn!(%error, "failed to acknowledge CodexMarathon recovery release");
            }
            return;
        }
