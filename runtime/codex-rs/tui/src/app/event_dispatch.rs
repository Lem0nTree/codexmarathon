...
fn recovery_lifecycle_notification(
    event: CodexMarathonRecoveryLifecycle,
) -> codex_app_server_protocol::ClientNotification {
    use codex_app_server_protocol::{
        ClientNotification, CodexMarathonRecoveryLifecycleNotification,
    };

    let notification = match event {
        CodexMarathonRecoveryLifecycle::Parked {
            recovery_id,
            thread_id,
            turn_id,
            source_account_id,
            reason,
        } => CodexMarathonRecoveryLifecycleNotification {
            event_type: "recovery_parked".to_string(),
            recovery_id,
            thread_id,
            turn_id,
            source_account_id,
        ...
        CodexMarathonRecoveryLifecycle::Started {
            recovery_id,
            thread_id,
            turn_id,
        } => CodexMarathonRecoveryLifecycleNotification {
        ...
        CodexMarathonRecoveryLifecycle::Completed {
            recovery_id,
            thread_id,
            turn_id,
            outcome,
...
impl App {
    /// Flush native recovery observations in creation order. A failed send is
    /// retained for the next app-server event, which makes reconnect/replay
    /// safe without asking the ChatWidget to create another prompt or queue.
    pub(super) async fn flush_codexmarathon_recovery_events(
        &mut self,
        app_server: &AppServerSession,
    ) {
        while let Some(event) = self.pending_codexmarathon_recovery_events.pop_front() {
            if let Err(error) = app_server
                .notify(recovery_lifecycle_notification(event.clone()))
                .await
            {
                tracing::warn!(%error, "failed to forward CodexMarathon recovery lifecycle event");
                self.pending_codexmarathon_recovery_events.push_front(event);
                break;
            }
        }
    }
