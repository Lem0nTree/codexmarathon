...
impl ChatWidget {
        ...
        match notification {
            ...
            ServerNotification::TurnStarted(notification) => {
                ...
                if self.marathon_recovery_controller_enabled
                    && self.active_usage_limit_resume_recovery_id.is_some()
                    && self
                        .active_usage_limit_resume_thread_id
                        .as_deref()
                        .is_none_or(|thread_id| thread_id == notification.thread_id)
                {
                    // Capture the server's actual synthetic turn ID for
                    // completion matching. The lifecycle correlation sent to
                    // Marathon remains the stable failed-turn ID captured at
                    // park time.
                    self.active_usage_limit_resume_dispatch_turn_id =
            ...
            ServerNotification::CodexMarathonRecoveryRelease(_) => {
                // The app-level dispatcher consumes this native command so it
                // can acknowledge it through the app-server client. It must
                // never enter the ordinary notification reducer.
            }
    ...
    pub(super) fn handle_turn_completed_notification(
        ...
        self.maybe_complete_usage_limit_resume(&notification, replay_kind);
    ...
    /// Emit one completion observation for the synthetic continuation that
    /// was dispatched by the normal input flow. Replay notifications are
    /// deliberately ignored: the adapter/bridge deduplicate live events, and
    /// replay must never manufacture a second lifecycle transition.
    fn maybe_complete_usage_limit_resume(
        &mut self,
        notification: &TurnCompletedNotification,
        replay_kind: Option<ReplayKind>,
    ) {
        if replay_kind.is_some() || !self.marathon_recovery_controller_enabled {
            return;
        }
        let Some(recovery_id) = self.active_usage_limit_resume_recovery_id.clone() else {
            return;
        };
        if self
            .active_usage_limit_resume_thread_id
            .as_deref()
            .is_some_and(|thread_id| thread_id != notification.thread_id)
        {
            return;
        }
        if self
            .active_usage_limit_resume_dispatch_turn_id
            .as_deref()
