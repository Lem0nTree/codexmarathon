    ...
    pub(super) fn on_rate_limit_error(&mut self, error_kind: RateLimitErrorKind, message: String) {
        ...
        if usage_limit_error {
                ...
                && let Some(prompt) = self.usage_limit_resume_prompt()
            {
                ...
                let recovery_id = thread_id
                    .as_deref()
                    .zip(turn_id.as_deref())
                    .map(|(thread_id, turn_id)| format!("usage-limit:{thread_id}:{turn_id}"));
                self.pending_usage_limit_resume_recovery_id = recovery_id.clone();
                self.pending_usage_limit_resume_thread_id = thread_id.clone();
                self.pending_usage_limit_resume_turn_id = turn_id.clone();
