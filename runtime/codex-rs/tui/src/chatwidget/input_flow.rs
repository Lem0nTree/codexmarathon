...
impl ChatWidget {
    ...
    pub(crate) fn maybe_send_next_queued_input(&mut self) -> bool {
        if !self.is_session_configured() || self.misalignment_policy_violation {
            return false;
        }
        ...
        if let Some(user_message) = self.pending_usage_limit_resume_turn.take() {
            let recovery_id = self.pending_usage_limit_resume_recovery_id.take();
            let recovery_thread_id = self.pending_usage_limit_resume_thread_id.take();
            let recovery_turn_id = self.pending_usage_limit_resume_turn_id.take();
            ...
            if accepted {
                // The native synthetic turn has crossed the existing submit
                // operation boundary. Emit started only now; taking the
                // pending value alone is not a dispatch.
                if self.marathon_recovery_controller_enabled
                    && let Some(recovery_id) = recovery_id.clone()
                {
                    self.active_usage_limit_resume_recovery_id = Some(recovery_id.clone());
                ...
                // already-created continuation. Restore the same pending
                // object and correlation; no replacement queue entry is made.
                self.pending_usage_limit_resume_turn = Some(user_message);
                self.pending_usage_limit_resume_recovery_id = recovery_id;
                self.pending_usage_limit_resume_thread_id = recovery_thread_id;
                self.pending_usage_limit_resume_turn_id = recovery_turn_id;
                self.usage_limit_resume_waiting_for_auth_reload = false;
            }
            self.refresh_pending_input_preview();
            return accepted;
