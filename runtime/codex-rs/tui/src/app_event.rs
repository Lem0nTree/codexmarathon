...
/// Metadata-only lifecycle observation for the embedded Marathon recovery
/// seam. The ChatWidget remains the owner of the pending synthetic turn; the
/// app layer only forwards these observations to app-server.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) enum CodexMarathonRecoveryLifecycle {
    Parked {
        recovery_id: String,
