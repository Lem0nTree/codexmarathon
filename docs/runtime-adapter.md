...
protocol v1. It is an implementation map, not an instruction to copy donor
code. The runtime remains the owner of authentication, turn safety, model
transports, and recovery; the Go controller only coordinates intent and
verifies observed state.

## Boundary and transport

The adapter speaks the shared schemas in `protocol/` over a bidirectional
stream (Unix socket, named pipe, or an in-process test pipe). Messages are
...
identity, and auth generation. No credentials, auth JSON, or authorization
headers are ever placed in protocol payloads or the event journal.

## Observed donor evidence

The two checkouts under `donor/` are reference-only. The following symbols are
the evidence for the adapter mapping:
...
Those ideas belong to the controller's account/credential packages. The
runtime adapter must not import or duplicate them.
...
| `CommitAuthTransition` | Require the exact pending ID/generation, then invoke the existing AuthManager reload path after controller atomic deployment. |
...
4. A changed auth reload uses Codext's existing model-transport invalidation
   path before `identity_changed`.
5. A sparse rate-limit notification retains the runtime connection identity
   and requests a full read when multiple by-ID buckets make attribution
   ambiguous.
6. Runtime recovery events are observed exactly once; no controller-generated
